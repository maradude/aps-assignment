...
def get_range(node, region):
    ...
    return SearchKDTreeNoContained(node, region, current_region=bounds, current_depth=0)
...
def SearchKDTreeNoContained(node, target_region, current_region, current_depth):
    ...
    if intersect(left_region, target_region):
        reported_nodes.extend(SearchKDTreeNoContained(
